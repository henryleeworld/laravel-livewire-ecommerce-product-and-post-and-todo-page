<div class="space-y-6">
    <div class="flex justify-between">
        <div class="space-x-8">
            <input wire:model.live="searchQuery" type="search" id="search" placeholder="{{ __('Search...') }}">
    ...
    <div class="text-red-600" wire:loading>{{ __('Loading...') }}</div>
...
{{--                            {{ $product->category->name }}--}}
                        ...
                        <td>
                            <a href="{{ route('products.edit', $product) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 rounded-md font-semibold text-xs text-white uppercase tracking-widest">
                                 {{ __('Edit') }} </a>
                            <a wire:click="deleteProduct({{ $product->id }})" onclick="return confirm('{{ __('Are you sure?') }}') || event.stopImmediatePropagation()" href="#" class="inline-flex items-center px-4 py-2 bg-red-600 rounded-md font-semibold text-xs text-white uppercase tracking-widest">
                                 {{ __('Delete') }} </a>
                        </td>
                    </tr>
                @empty
